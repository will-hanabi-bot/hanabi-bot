...
export default class HGroup extends Game {
	...
	constructor(tableID, state, in_progress, level = 1) {
		...
		this.players = this.players.map(p =>
			new HGroup_Player(p.playerIndex, p.all_possible, p.all_inferred, p.hypo_stacks, p.hypo_plays, p.thoughts, p.links, p.play_links, p.unknown_plays, p.waiting_connections, p.elims));

		const c = this.common;
		this.common = new HGroup_Player(c.playerIndex, c.all_possible, c.all_inferred, c.hypo_stacks, c.hypo_plays, c.thoughts, c.links, c.play_links, c.unknown_plays, c.waiting_connections, c.elims);
